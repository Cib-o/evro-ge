...
// ── ისტორიული გრაფიკი ─────────────────────────────────────────────────────────
// SVG ცარიელი მოდის — `d`-ს და მინ/მაქს რიცხვებს **Worker ავსებს ედჯზე**
// (src/index.js → ChartHandler), ისევე როგორც კურსს. build-time-ზე ჩაბეჭდვა
// ნიშნავდა, რომ ხაზი ბოლო ბილდზე გაიყინებოდა — მონაცემი ხომ ყოველდღე ახლდება.
//
// i18n: <svg> OPAQUE-ია, ანუ არ ითარგმნება. caption-ის რიცხვები ცალკე <span>-ებშია,
// ამიტომ შაბლონი ხდება „მინიმუმი {0} ₾, მაქსიმუმი {1} ₾…" — ერთი key ყველა ვალუტაზე.
function chartBlock(gen) {
  return `    <div class="chartbox">
      <h2>${gen} კურსი ბოლო 12 თვეში</h2>
      <svg class="chart" viewBox="0 0 680 180" width="680" height="180" aria-hidden="true" focusable="false">
        <path class="chart-area" data-chart="area" d=""></path>
        <path class="chart-line" data-chart="path" d=""></path>
      </svg>
      <p class="chart-cap">მინიმუმი <span class="num" data-chart="min">—</span> ₾, მაქსიმუმი <span class="num" data-chart="max">—</span> ₾, ცვლილება წელიწადში <span class="num" data-chart="chg">—</span></p>
    </div>
`;
}
...
/* ისტორიული გრაფიკი — path-ს Worker ავსებს ედჯზე (data-chart), იხ. src/index.js */
.chartbox{margin-top:34px}
.chartbox h2{font-family:"Noto Serif Georgian",serif;font-size:22px;margin-bottom:12px;font-weight:700}
.chart{display:block;width:100%;height:auto;background:var(--panel);border:1px solid var(--line);border-radius:var(--radius)}
.chart-area{fill:rgba(46,84,184,.10);stroke:none}
.chart-line{fill:none;stroke:var(--euro);stroke-width:2;stroke-linejoin:round;stroke-linecap:round;vector-effect:non-scaling-stroke}
.chart-cap{margin-top:10px;font-size:13.5px;color:var(--muted)}
.chart-cap .num{color:var(--ink);font-weight:500}
...
.foot .gh{display:inline-flex;align-items:center;gap:5px;color:var(--euro);font-weight:600;text-decoration:none;margin-top:10px}
.foot .gh:hover{color:var(--euro-2)}
.foot .gh svg{width:15px;height:15px}
...
// Google Analytics (gtag.js) — ყველა გვერდის <head>-ში.
const GA_TAG = `<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-JV4KXX3MYF"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'G-JV4KXX3MYF');
</script>`;

// ღია კოდის (open source) ბმული — ფუტერში ყველა გვერდზე.
const GITHUB_LINK = `<a class="gh" href="https://github.com/Cib-o/evro-ge" target="_blank" rel="noopener" aria-label="საიტის ღია კოდი GitHub-ზე"><svg viewBox="0 0 16 16" aria-hidden="true"><path fill="currentColor" d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z"/></svg> ღია კოდი</a>`;
...
const SSR_HELPERS_JS = `
  ...
  function evalSSR(spec,map){var x=/^(?:(\\d+(?:\\.\\d+)?)([*\\/]))?([A-Z]{3})(?:\\/([A-Z]{3}))?$/.exec(spec);if(!x)return null;var r=map[x[3]];if(r==null||!isFinite(r))return null;if(x[4]){var d=map[x[4]];if(d==null||!isFinite(d)||d===0)return null;r=r/d;}if(!x[1])return r;var a=parseFloat(x[1]);return x[2]==="*"?a*r:a/r;}
...
// ── ცალკეული ვალუტის სადესანტო გვერდები (GBP/TRY/RUB) ────────────────────────
// "ფუნტის კურსი", "ლირის კურსი", "რუბლის კურსი" — ე.წ. "მსგავსი სიტყვები".
const LANDINGS = [
  {
    code: "GBP", slug: "funtis-kursi", full: "ბრიტანული ფუნტი",
    nom: "ფუნტი", gen: "ფუნტის", loc: "ფუნტში", a: "ფუნტია", latin: "funtis kursi",
    title: "ფუნტის კურსი დღეს — GBP/GEL ოფიციალური კურსი | evro.ge",
    og: "ფუნტის კურსი დღეს — GBP/GEL ოფიციალური კურსი",
    h1: "ფუნტის კურსი დღეს — ფუნტი ლარი",
    keywords: "ფუნტის კურსი, ფუნტი ლარი, funtis kursi, ბრიტანული ფუნტი, GBP GEL",
  ...
  },
  {
    ...
    h1: "ლირის კურსი დღეს — თურქული ლირა ლარი",
    keywords: "ლირის კურსი, თურქული ლირა, liris kursi, ლირა ლარი, TRY GEL",
  ...
  },
  {
    ...
    h1: "რუბლის კურსი დღეს — რუსული რუბლი ლარი",
    keywords: "რუბლის კურსი, რუსული რუბლი, rublis kursi, რუბლი ლარი, RUB GEL",
    context: "რუბლის კურსი ხშირად აქტუალურია ფულადი გზავნილების დროს. ეროვნული ბანკი რუბლის ოფიციალურ კურსს ყოველ სამუშაო დღეს ადგენს, ბანკებსა და გადამცვლელ პუნქტებში კი კურსი ოდნავ განსხვავდება.",
  },
];

function buildCurrencyLanding(cfg) {
  const canonical = `${SITE}/${cfg.slug}/`;
  const desc = `${cfg.gen} კურსი ლარში დღეს — 1 ${cfg.nom} რამდენი ლარია ეროვნული ბანკის ოფიციალური კურსით. გადათვალე ${cfg.full} ლარში (${cfg.latin}).`;

  const faq = [
    { q: `რა ღირს 1 ${cfg.nom} დღეს?`, a: `1 ${cfg.gen} დღევანდელი ოფიციალური კურსი ლარში ნაჩვენებია გვერდის თავში, საქართველოს ეროვნული ბანკის მონაცემებით. კურსი ავტომატურად ახლდება.` },
...
<meta property="og:site_name" content="evro.ge">
<meta property="og:locale" content="ka_GE">
<meta property="og:title" content="${esc(cfg.og)}">
<meta property="og:description" content="${esc(desc)}">
<meta property="og:url" content="${canonical}">
<meta property="og:image" content="${SITE}/og.svg">
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:title" content="${esc(cfg.og)}">
<meta name="twitter:description" content="${esc(desc)}">
<meta name="twitter:image" content="${SITE}/og.svg">
${FONT_LINKS}
<script type="application/ld+json">${graphLd}</script>
...
.prose a{color:var(--euro);font-weight:600;text-decoration:none}
.chips{display:flex;flex-wrap:wrap;gap:10px}
.chip{background:var(--panel);border:1px solid var(--line);border-radius:999px;padding:8px 14px;font-size:14px;text-decoration:none;font-weight:500}
.chip:hover{border-color:var(--euro);color:var(--euro)}
</style>
</head>
<body>
...
<main>
  <div class="wrap">
    <h1>${cfg.h1}</h1>
    <p class="sub">${cfg.full} (${cfg.code}) ოფიციალური კურსი ლარში დღეს, საქართველოს ეროვნული ბანკის მონაცემებით.</p>
    ...
    <div class="conv">
      <div class="conv-cell">
        <label for="curIn">${cfg.full} (${cfg.code})</label>
        <input class="conv-input num" id="curIn" inputmode="decimal" value="100">
      </div>
      <div class="conv-eq">=</div>
      <div class="conv-cell">
        <label for="gelIn">ლარი (GEL)</label>
        <input class="conv-input num" id="gelIn" data-ssr="100*${cfg.code}" data-dp="2" inputmode="decimal" value="">
      </div>
    </div>
...
${popularBlock}
    ...
    <div class="prose">
      <h2>${cfg.gen} კურსი ლარში დღეს</h2>
...
(function(){
  ...
  var rate=null;
  var dateEl=document.getElementById("date"), sourceEl=document.getElementById("source"),
      curIn=document.getElementById("curIn"), gelIn=document.getElementById("gelIn");
${SSR_HELPERS_JS}
  function parseNum(s){return parseFloat(String(s).replace(/[^\\d.,-]/g,"").replace(",","."));}
  function recalc(from){
    if(!rate) return;
    if(from==="cur"){var u=parseNum(curIn.value);gelIn.value=isFinite(u)?fmt(u*rate,2):"";}
    else{var g=parseNum(gelIn.value);curIn.value=isFinite(g)?fmt(g/rate,2):"";}
  }
  function paint(o){ rate=o.map[CODE]; fillSSR(o.map); if(dateEl) dateEl.textContent=o.date?("კურსი "+o.date):"განახლდა"; if(sourceEl) sourceEl.textContent=o.src; }
  function fail(){ var h=document.getElementById("heroRate"); if(h){h.textContent="ვერ ჩაიტვირთა";h.style.fontSize="clamp(28px,7vw,44px)";} if(dateEl) dateEl.textContent=""; }
  curIn.addEventListener("input",function(){recalc("cur");});
  gelIn.addEventListener("input",function(){recalc("gel");});
  (function go(i){
    if(i>=SOURCES.length){fail();return;}
    var s=SOURCES[i];
...
function buildHubPage() {
  ...
  const rows = [
    ...
    { code: "GBP", name: "ბრიტანული ფუნტი", href: "/funtis-kursi/" },
    { code: "TRY", name: "თურქული ლირა", href: "/liris-kursi/" },
    { code: "RUB", name: "რუსული რუბლი", href: "/rublis-kursi/" },
  ]
    .map(
      (r) =>
